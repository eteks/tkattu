...
if ($_GET["action"] == "privilege_roles_add" || $_GET["action"] == "privilege_roles_edit") {
    ...
    $admin_role_master_values=(isset($admin_role_master_values) && !empty($admin_role_master_values) ? ($admin_role_master_values) : '');
   $parentid= (isset($_REQUSEST['parent_id']) && empty($_REQUSEST['parent_id']) ? $_REQUSEST['parent_id'] : "");
    if (!empty($admin_role_master_values["parent_id"])) {
        $parent_id = (!empty($parentid) ? $parentid:$admin_role_master_values["parent_id"]);
    }$admin_role_master_values["sort_order"] = (isset($admin_role_master_values["sort_order"]) && !empty($admin_role_master_values["sort_order"]) ? $admin_role_master_values["sort_order"]:"");

    if (!not_null ($admin_role_master_values["sort_order"])) {
        $admin_role_master_values["sort_order"] = '';
    }
    $admin_role_master_values["active"] = (isset($admin_role_master_values["active"]) && !empty($admin_role_master_values["active"]) ? $admin_role_master_values["active"]:"");
...
?>
  ...
  <form name="frmPrivilegeAdminRoles" method="POST" enctype="multipart/form-data" action="<?php echo href_link(FILENAME_PRIVILEGE_ROLES); ?>">  
    ...
    <tr><td></td></tr>
	<tr>
		  <td style="padding-left:6px" class="heading_light_blue"><img src="images/arrow_heading.gif" />&nbsp;<?php echo TITLE_MANAGE_PRIVILEGE_ROLES;?></td>
		  <td align="left"></td>
	</tr>
    <tr>
    ...
    <tr>
        ...
        <td colspan="4"><input name="role_name" type="text" id="role_name" tabindex="2" class="tahoma12blacknormal padding_left" dir="ltr" size="60" value="<?php $admin_role_master_values["name"] = (isset($admin_role_master_values["name"]) && !empty($admin_role_master_values["name"]) ? $admin_role_master_values["name"]:""); echo stripslashes($admin_role_master_values["name"]); ?>" ONCHANGE="validatePresent(this, 'error_rolename');">&nbsp;<span id="error_rolename" class="tahoma10rednormal"><?php $admin_role_master_values["name"] = (isset($admin_role_master_values["name"]) && !empty($admin_role_master_values["name"]) ? $admin_role_master_values["name"]:""); if(!not_null($admin_role_master_values["name"])) { ?>Required<?php } ?></span></td>
    </tr>
    <tr>
        <td width="20%" class="tahoma12blacknormal padding_left">Short Description:</td>
        <td colspan="4"><textarea name="short_desc" id="short_desc" rows="03" class="textbox" class="tahoma12blacknormal padding_left" cols="72" tabindex="3"><?php $admin_role_master_values["short_descr"] = (isset($admin_role_master_values["short_descr"]) && !empty($admin_role_master_values["short_descr"]) ? $admin_role_master_values["short_descr"]:"");   print stripslashes($admin_role_master_values["short_descr"])?></textarea></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td width="3%"><input name="active" class="noneborder" type="radio" tabindex="5" value="Y" <?php if(isset($Y) && !empty($Y)) echo $Y ; ?>></td>
        <td width="4%" class="tahoma12blacknormal padding_left">Yes</td>
        <td width="3%"><input name="active" class="noneborder" type="radio" tabindex="6" value="N" <?php if(isset($N) && !empty($N)) echo $N;?>></td>
    ...
    <tr>
        ...
        <td colspan="4"><?php if ($admin_role_add_edit_user_allow) { ?><input name="buttonSubmit" style="font-size:11px; color:#e1e1e1; background:#666666; border:1px solid #000; padding:1px 3px 1px 3px;cursor:pointer;" type="button" tabindex="6" title="Submit" onClick="submitfrmPrivilegeAdminRoles(document.frmPrivilegeAdminRoles);" value="Submit">&nbsp;<?php } ?>
            <input name="buttonCancel" type="button" tabindex="7" title="Cancel" value="Cancel" style="font-size:11px; color:#e1e1e1; background:#666666; border:1px solid #000; padding:1px 3px 1px 3px;cursor:pointer;" ONCLICK="document.location.href='<?php echo href_link(FILENAME_PRIVILEGE_ROLES, 'parentid='.$_GET["parentid"]); ?>'"></td>
    </tr>
 </table>
 </form>
<?php
    ...
    $_GET['parentid'] = (isset($_GET['parentid']) ? trim($_GET['parentid']) : '');
...
?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
        ...
        ?>
        <tr>
            <td><img src="images/newfolder.gif" width="16" height="16"  align="absbottom" alt="Add Role">&nbsp;&nbsp;<a class="tahoma12blacknormal" title="add new module" href="<?php echo href_link(FILENAME_PRIVILEGE_ROLES, 'action=privilege_roles_add&parentid='.$parentid.''); ?>"><b>Add Roles</b></a></td>
        </tr>
        <?php
            }
        ?>
