...
if ($action == "department_add" || $action == "department_edit") {
	$department_add = "department_insert";
	$department_edit = "department_update";
if ($action == "department_edit" ) {

    $departmentEntry_result = $hms_info_obj->departmentEntrySingRec();
    $departmentEntry_values = db_fetch_array ($departmentEntry_result);
}
    if ( not_null ( $departmentEntry_values["active"] ) ) ${$departmentEntry_values["active"]}= "checked";
    else $Y = "checked";

?>
<form name="frmStudentUser" method="POST" enctype="multipart/form-data" action="<?php echo href_link(FILENAME_DEPARTMENT_CREATION); ?>">
   ...
   <input type="hidden" name="page" value="<?php echo (isset($page) ? $page : 1); ?>">
    ...
    <tr><td></td></tr>
	      <tr>
            <td colspan="6" align="left" style="background-image:url(images/back_header_bg.jpg); background-repeat:repeat-x; padding-top:8px; padding-left:0px"><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td align="left" class="heading_light_blue" valign="top">&nbsp;<?php echo TITLE_MANAGE_DEPARTMENT_CREATION;?></td>
              </tr>
            </table></td>
        </tr>
        <tr>
        ...
        <tr>
            <td width="20%" class="tahoma12blacknormal padding_left">Department Name:</td>
            <td width="87%" colspan = "2" class="tahoma12blacknormal padding_left">
             <input name="department_name" type="text"  id="department_name" tabindex="1" dir="ltr" size="60%" value="<?php print stripslashes($departmentEntry_values["department_creation_name"]); ?>" ONCHANGE="validatePresent(this, 'error_department_name');">&nbsp;<span id="error_department_name" class="tahoma10rednormal"><?php if (! not_null ($departmentEntry_values["department_creation_name"]) ) { ?>Required<?php } ?>
          </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td >
                ...
                <input class="btn_style1" name="buttonCancel" type="button" tabindex="5" title="Cancel" value="Cancel" ONCLICK="document.location.href='<?php echo href_link(FILENAME_DEPARTMENT_CREATION, 'page=' . $_GET["page"]); ?>'" style="font-size:11px; color:#e1e1e1; background:#666666; border:1px solid #000; padding:1px 3px 1px 3px;cursor:pointer;"></td>
       </tr>
    </table>
</form>
<?php
