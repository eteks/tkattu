...
class hmsprivilegeUserClass {

 function privilegeUserInsert () {

    $privilege_user_insert_sql = "INSERT INTO " . TABLE_HMS_ADMIN_MASTER . " ( `id`, `username`, `password`, `first_name` , `last_name`, `email`, `admin_role_mst_id`,`active` , `date_added` , `date_modified` ) VALUES ( '', '" . addslashes($_POST["uname"]) . "', '" . addslashes($_POST["pword"]) . "', '" . addslashes($_POST["fname"]) . "', '" . addslashes($_POST["lname"]) . "', '" . addslashes($_POST["email"]) . "', '" . $_POST["parent_id"] . "','" . $_POST["active"] . "', NOW(), NULL)";
    $privilege_user_insert = db_query ($privilege_user_insert_sql);
	//$privilege_user_insert_id = db_insert_id(); // this will not work if field datatype is BIGINT
    $last_insert_id = db_query("SELECT LAST_INSERT_ID() as insert_id");
    $last_insert_values = db_fetch_array($last_insert_id);
    $privilege_user_insert_id = $last_insert_values['insert_id'];
    if (not_null($_POST["admin_modules"]) && count($_POST["admin_modules"]) > 0) {
        ...
        $privilege_user_module_insert_sql = "INSERT INTO " . TABLE_HMS_MODULE_ADMIN . " (`admin_modules_mst_id`, `admin_mst_id`) " . $selected_modules_str;
    ...
    if (not_null($_POST["admin_events"]) && count($_POST["admin_events"]) > 0) {
        ...
        $privilege_user_event_insert_sql = "INSERT INTO " . TABLE_HMS_MODULE_EVENT_ADMIN . " (`admin_modules_events_mst_id`, `admin_mst_id`) " . $selected_events_str;
        db_query($privilege_user_event_insert_sql);
    }
	return $privilege_user_insert_id;
 }

 function privilegeUserUpdate () {

  if( not_null($_GET["id"]) && $_GET["id"]>0 ) {
        $privilege_user_update_sql = "UPDATE " . TABLE_HMS_ADMIN_MASTER . " SET  `admin_role_mst_id` = '" . $_GET["parent_id"] . "', `username` = '" . addslashes($_POST["uname"]) . "', `password` = '" . addslashes($_POST["pword"]) . "', `first_name` = '" . addslashes($_POST["fname"]) . "', `last_name` = '" . addslashes($_POST["lname"]) . "', `email` = '" . addslashes($_POST["email"]) . "', `active` = '" . $_POST["active"] . "', `date_modified` = NOW()  WHERE id = '". $_GET["id"] ."'";
        $privilege_user_update = db_query ($privilege_user_update_sql);

        db_query ("DELETE FROM " . TABLE_HMS_MODULE_ADMIN . " WHERE  `admin_mst_id` = '" . $_GET["id"] . "'");
        if(not_null($_POST["admin_modules"]) && count($_POST["admin_modules"]) > 0) {
            $selected_modules_str = "VALUES ('" . implode("', '" . $_GET["id"] . "'), ('", $_POST["admin_modules"]) . "', '" . $_GET["id"] . "');";
            $privilege_user_module_insert_sql = "INSERT INTO " . TABLE_HMS_MODULE_ADMIN . " (`admin_modules_mst_id`, `admin_mst_id`) " . $selected_modules_str;
            db_query($privilege_user_module_insert_sql);
        }
        db_query ("DELETE FROM " . TABLE_HMS_MODULE_EVENT_ADMIN . " WHERE  `admin_mst_id` = '" . $_GET["id"] . "'");
        if (not_null($_POST["admin_events"]) && count($_POST["admin_events"]) > 0) {
 ...
 function privilegeUserDelete () {
     if (not_null($_POST["id"]) && $_POST["id"] > 0) {
            db_query ("DELETE from " . TABLE_HMS_MODULE_EVENT_ADMIN . "  WHERE  admin_mst_id = '" . $_POST["id"] . "'");
            db_query ("DELETE from " . TABLE_HMS_MODULE_ADMIN . "  WHERE  admin_mst_id = '" . $_POST["id"] . "'");
            db_query ("DELETE from " . TABLE_HMS_ADMIN_MASTER . "  WHERE  id = '" . $_POST["id"] . "'");
     }
 }
 ...
 function privilegeUserDuplication () {
     $uname = urldecode($_GET["uname"]);
      ...
      $privilege_user_duplication_sql = "SELECT id FROM " . TABLE_HMS_ADMIN_MASTER . " WHERE username = '" . trim($uname) . "'" . $extra_cond . "";
...
{
    $modules_id_str='';
    $privilege_users_module_result = db_query("SELECT admin_modules_mst_id FROM " . TABLE_HMS_MODULE_ADMIN ." WHERE admin_mst_id = '" . $_GET["id"] . "'");
...
{
    $events_id_str='';
    $privilege_users_event_result = db_query("SELECT admin_modules_events_mst_id FROM " . TABLE_HMS_MODULE_EVENT_ADMIN ." WHERE admin_mst_id = '" . $_GET["id"] . "'");
...
function get_all_roles_modulenames($id)
{
    ...
    $privilege_roles_module_result = db_query("SELECT admin_modules_mst_id, name FROM " . TABLE_HMS_MODULE_ROLE .", " . TABLE_HMS_MODULE_MASTER . " WHERE " .  TABLE_HMS_MODULE_MASTER . ".id = admin_modules_mst_id AND active='Y' AND admin_role_mst_id = '" . $id . "' AND parent_id = 0 ORDER BY sort_order");
...
function get_all_child_modules($parent_id, $role_id)
{
    ...
    if($parent_id>0 && $this->no_of_child_of($parent_id, TABLE_HMS_MODULE_MASTER)>0) {
        //get all child list
        $privilege_module_child_result = db_query("SELECT admin_modules_mst_id, name FROM " . TABLE_HMS_MODULE_MASTER .", " . TABLE_HMS_MODULE_ROLE . " WHERE parent_id = '" . (int)$parent_id . "' AND  " .  TABLE_HMS_MODULE_MASTER . ".id = admin_modules_mst_id AND admin_role_mst_id = '" . (int)$role_id . "' ORDER BY sort_order");
        $i=0;
...
function get_all_event_names($module_id)
{
    ...
    $privilege_module_event_result = db_query("SELECT id, name FROM " . TABLE_HMS_MODULE_EVENT_MASTER ." WHERE admin_modules_mst_id = '" . $module_id . "'");
    //if found then
    ...
    if(not_null($role_id)) {
        ...
        if(count($modules_ids_arr)>0) {
            ...
            foreach($modules_ids_arr as $value) {
                $get_str_tree .='<tr><td class="tahoma12blacknormal padding_left">';
        ...
        if(count($events_ids_arr)>0) {
            ...
            $get_str_tree.='<table width="100%" align="left" cellpadding="0" cellspacing="0"><tr><td class="tahoma12blacknormal padding_left"><b>Events</b></tr></td>';
            foreach($events_ids_arr as $event_value) {
                $get_str_tree.='<tr><td width="100%" class="tahoma12blacknormal padding_left">';
...
function get_user_role_id($user_id) {
    $get_user_role_result = db_query("SELECT admin_role_mst_id FROM " . TABLE_HMS_ADMIN_MASTER ." WHERE  id = '" . $user_id . "'");
 ...
 function privilegeUpdateActive () {
	 if(ADVERTISE_ADVERTISER_ROLE_ID == $this->get_user_role_id($_GET["id"])) {
        $privilege_user_update_sql = "UPDATE " . TABLE_ADVERTISE_ADVERTISER_MASTER . " aam, " . TABLE_HMS_ADMIN_MASTER . " am SET am.active = '" . $_GET["value"] . "', am.date_modified = NOW(), aam.active = '" . $_GET["value"] . "', aam.date_modified = NOW() WHERE am.id = '" . (int)$_GET["id"] . "' AND am.id = aam.admin_mst_id";
    } else {
        $privilege_user_update_sql = "UPDATE " . TABLE_HMS_ADMIN_MASTER . " SET active = '" . $_GET["value"] . "', date_modified = NOW() WHERE id = '" . (int)$_GET["id"] . "'";
    }
    $privilege_user_update = db_query($privilege_user_update_sql);
 }
...
function privilegeUserMastreTotalSql ($user_sql_cond) {
    $user_master_total_sql = "SELECT id, username, first_name, last_name, email, active ,date_added, date_modified FROM " . TABLE_HMS_ADMIN_MASTER . " WHERE " . $user_sql_cond . " ORDER BY username"; 
...
function get_privilege_role_tree($parent_id = '0', $spacing = '', $exclude = '', $role_tree_array = '', $include_itself = false)
{
 ...
 function totalDbUserMasterNo() {
     ...
     $total_db_user_master_no = db_num_rows(db_query("SELECT id FROM " . TABLE_HMS_ADMIN_MASTER ));
     return $total_db_user_master_no;
}

 function privilegeUserMasterSql($id) {
            $privilege_user_master_sql = "SELECT id, admin_role_mst_id, username, password, first_name, last_name, email, active FROM " . TABLE_HMS_ADMIN_MASTER . " WHERE id = '" . $id . "'";
