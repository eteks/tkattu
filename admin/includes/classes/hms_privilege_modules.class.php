...
class UserprivilegeModulesClass {
...
function privilegeModuleInsert ($parent_id, $sort_order, $name, $short_descr, $filename, $parameters, $active) {
    ...
    if (!not_null($sort_order)) {
        $sort_order = $this->getPrivilegeMaxSortOrder(TABLE_HMS_MODULE_MASTER, $module_sql_cond ) + 1 ;
    } else {
        db_query ("UPDATE " . TABLE_HMS_MODULE_MASTER . " SET sort_order = (sort_order + 1) WHERE sort_order >= '" . $sort_order . "' AND " . $module_sql_cond);
    }
    db_query ("INSERT INTO " . TABLE_HMS_MODULE_MASTER . " ( `id`, `parent_id`, `name`, `short_descr`, `filename`, `parameters`, `sort_order`, `active` , `date_added` , `date_modified` ) VALUES ( '', '" . $parent_id . "', '" . addslashes($name) . "', '". addslashes($short_descr) . "', '" . addslashes($filename) . "', '" . addslashes($parameters) . "', '" . $sort_order . "', '" . $active . "', NOW(), NULL)");
    ...
    if (not_null($id)) {
        ...
        if (!not_null($sort_order)) {
            if (not_null($previous_parent_id) && $previous_parent_id == $parent_id) {
                ...
                $this->updateRemainingPrivilegeSortorder($id, TABLE_HMS_MODULE_MASTER, $previous_parent_cond);
                $sort_order = $this->getPrivilegeMaxSortOrder(TABLE_HMS_MODULE_MASTER, $module_sql_cond) + 1;
            } else {
                $sort_order = $this->getPrivilegeMaxSortOrder   (TABLE_HMS_MODULE_MASTER, $module_sql_cond) + 1;
            }
            ...
            if (not_null($previous_parent_id) && $previous_parent_id != $parent_id) {
                ...
                db_query ("UPDATE " . TABLE_HMS_MODULE_MASTER . " SET sort_order = IF ('" . (int)$sort_order . "' < '" . (int)$previous_sort_order . "', (sort_order + 1), (sort_order - 1)) WHERE IF ('" . (int)$sort_order . "' < '" . (int)$previous_sort_order . "', sort_order >= '" . (int)$sort_order . "' AND sort_order <= '" . (int)$previous_sort_order . "', sort_order >= '" . (int)$previous_sort_order . "' AND sort_order <= '" . (int)$sort_order . "') AND " . $module_sql_cond);
            }
        }
        db_query ("UPDATE " . TABLE_HMS_MODULE_MASTER . " SET `parent_id` = '" . (int)$parent_id . "' , `name` = '" . addslashes($name) . "', `short_descr` ='". addslashes($short_descr) . "', `filename` = '" . addslashes($filename) . "', `parameters` = '" . addslashes($parameters) . "', `sort_order` = '" . (int)$sort_order . "' , `active` = '" . $active . "' , `date_modified` = NOW() WHERE  id = '" . (int)$id . "'");
    }
}
...
{
    $privilege_module_event_result = db_query("SELECT id FROM " . TABLE_HMS_MODULE_EVENT_MASTER ." WHERE admin_modules_mst_id = '" . $module_id . "'");
...
{
    $privilege_admin_module_event_delete = db_query("DELETE FROM " . TABLE_HMS_MODULE_EVENT_MASTER ." WHERE admin_modules_events_mst_id IN ('" . $string_event_arr . "')");
    $privilege_admin_module_event_delete = db_query("DELETE FROM " . TABLE_HMS_MODULE_EVENT_MASTER ." WHERE id IN ('" . $string_event_arr . "')");
}

function getModulesForDelete($id)
{
    $all_id='';
    if($id>0 && $this->noOfChildOf($id, TABLE_HMS_MODULE_MASTER)>0) {
        $privilege_module_child_result = db_query("SELECT id FROM " . TABLE_HMS_MODULE_MASTER ." WHERE parent_id = '" . $id . "'");
...
{
    $privilege_module_event_delete = db_query("DELETE FROM " . TABLE_HMS_MODULE_EVENT_MASTER ." WHERE admin_modules_mst_id IN ('" . $string_module_arr . "')");
    $privilege_module_admin_delete = db_query("DELETE FROM " . TABLE_HMS_MODULE_ADMIN ." WHERE admin_modules_mst_id IN ('" . $string_module_arr . "')");
    $privilege_module_role_delete = db_query("DELETE FROM " . TABLE_HMS_MODULE_ROLE ." WHERE admin_modules_mst_id IN ('" . $string_module_arr . "')");
}
...
function privilegeModuleDelete($id, $parent_id) {
        ...
        if (not_null($string_module_arr)) {
            ...
            $this->update_remaining_privilege_sortorder($id, TABLE_HMS_MODULE_MASTER, $module_sql_cond);
            db_query ("DELETE from " . TABLE_HMS_MODULE_MASTER . "  WHERE  id IN ('" . $string_module_arr . "')");
        }
    }
}

function privilegeModuleUpdateActive ($value, $id) {
    $privilege_module_update_sql = "UPDATE " . TABLE_HMS_MODULE_MASTER. " SET `active` = '" . $value . "', `date_modified` = NOW() WHERE `id` = " . $id ;
...
function privilegeModuleUpdateSortOrder($id, $value) {
     ...
     $previous_sort_order = $this->getPrivilegeSortOrder ($id, TABLE_HMS_MODULE_MASTER);
    if ($value == "d") $sort_order = $previous_sort_order + 1;
    else $sort_order = $previous_sort_order - 1;

    $swap_id = $this->getPrivilegeId($sort_order,TABLE_HMS_MODULE_MASTER, $module_sql_cond);

    $privilege_module_update_sql = "UPDATE " . TABLE_HMS_MODULE_MASTER . " SET `sort_order` = '" . $previous_sort_order  . "', `date_modified` = NOW() WHERE `id` = '" . $swap_id . "'";
    $privilege_module_update = db_query ($privilege_module_update_sql);

    $privilege_module_update_sql = "UPDATE " . TABLE_HMS_MODULE_MASTER . " SET `sort_order` = '" . $sort_order  . "', `date_modified` = NOW() WHERE `id` = '" . $id . "'";
    $privilege_module_update = db_query ($privilege_module_update_sql);
}

function modulesMasterSql($module_sql_cond) {
    $modules_master_sql = "SELECT id, name, filename, parameters, sort_order, active, date_added, date_modified FROM " . TABLE_HMS_MODULE_MASTER . " WHERE " . $module_sql_cond . " ORDER BY sort_order, name";
	//exit;
...
{
    $module_master_sql = "SELECT id, name, parent_id from " . TABLE_HMS_MODULE_MASTER . " WHERE id = '" . $parent_id . "'";
...
function privilegeModuleMasterSql ($id) {
            $privilege_module_master_sql = "SELECT id, parent_id, name, short_descr, filename, parameters, sort_order, active FROM " . TABLE_HMS_MODULE_MASTER . " WHERE id = " . $id ;
