...
class privilegeModuleEventsClass {
    ...
    function privilegeModuleEventInsert ($module_event_parent_id, $event_name, $short_desc, $event_filename, $event_parameters) {
        $admin_module_event_mst_insert_sql = "INSERT INTO " . TABLE_MODULE_EVENT_MASTER . " ( `id` ,`admin_modules_mst_id`, `name`, `short_descr`, `filename`, `parameters`) VALUES ( '', ". $module_event_parent_id.", '" . $event_name . "', '".$short_desc."', '".$event_filename."', '" . $event_parameters . "')";
    ...
    function privilegeModuleEventUpdate($module_event_parent_id, $event_name, $short_desc, $event_filename, $event_parameters, $id) {
       $admin_module_event_mst_update_sql = "UPDATE ". TABLE_MODULE_EVENT_MASTER." SET admin_modules_mst_id= " . $module_event_parent_id . ", name = '" . $event_name . "', short_descr = '" . $short_desc . "', filename = '" . $event_filename . "', parameters = '" . $event_parameters . "' WHERE id = '" . $id . "'";
       $admin_module_event_mst_update     = db_query ($admin_module_event_mst_update_sql); 

    }

    function privilegeModuleEventDelete ($id) {
       $admin_module_event_mst_delete_sql = "DELETE FROM ". TABLE_MODULE_EVENT_MASTER." WHERE id = '" . $id . "'";
       $admin_module_event_mst_delete     = db_query($admin_module_event_mst_delete_sql);
       $admin_module_event_admin_delete_sql = "DELETE FROM ". TABLE_MODULE_EVENT_ADMIN." WHERE admin_modules_events_mst_id = '" . $id . "'";
    ...
    function modulesEventMasterSql($event_module_parent_id) {
        $modules_event_master_sql = "SELECT id,admin_modules_mst_id, name, filename, parameters, short_descr, filename FROM " . TABLE_MODULE_EVENT_MASTER . " WHERE `admin_modules_mst_id`=" . $event_module_parent_id. " order by name";
