...
class privilegeRolesClass {
    ...
    function privilegeRolesInsert () {
        ...
        $admin_role_mst_max_sort_order_sql = "SELECT MAX(sort_order) as sort_order FROM " . TABLE_ROLE_MASTER . " WHERE parent_id = " . $_POST["role_parent_id"] . " ";
        ...
        $admin_role_master_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order + 1) WHERE  sort_order >= '" . $_POST["sort_order"] . "' AND parent_id = " . $_POST["role_parent_id"] . " ";
        $admin_role_master_update_sort_order     = db_query($admin_role_master_update_sort_order_sql);
		$sort_order = $_POST["sort_order"];
    }
    $admin_role_insert_sql = "INSERT INTO " . TABLE_ROLE_MASTER . " (`id`, `parent_id`, `name`, `short_descr`, `sort_order`, `active`, `date_added`) VALUES ('', " . $_POST["role_parent_id"] . ", '" . addslashes($_POST["role_name"]) . "', '" . addslashes($_POST["short_desc"]) . "', " . $sort_order . ", '" . $_POST["active"] . "', NOW())";
    ...
    if (isset ($_POST["parent"])) {
        ...
        $admin_module_to_admin_role_insert_sql = "INSERT INTO " . TABLE_MODULE_ROLE . "(`admin_modules_mst_id`, `admin_role_mst_id`) VALUES " . $role_string . " ";
...
{
    $adminrole_mst_query = db_query("select sort_order from " . TABLE_ROLE_MASTER . " where id = '" . (int)$id . "'");
...
{
    $adminrole_mst_query = db_query("select parent_id from " . TABLE_ROLE_MASTER . " where id = '" . (int)$id . "'");
    $adminrole_mst       = db_fetch_array($adminrole_mst_query);
  ...
  function privilege_roles_update ($sort_order, $role_parent_id, $role_name, $short_desc, $active, $parent, $id) {
    ...
    if (!not_null($sort_order)) {
        ...
        $admin_role_mst_max_sort_order_sql = "SELECT MAX(sort_order) as sort_order FROM " . TABLE_ROLE_MASTER . " WHERE parent_id = " . $role_parent_id . " ";
        $admin_role_mst_max_sort_order = db_query($admin_role_mst_max_sort_order_sql);
        $admin_role_mst_max_sort_order_values = db_fetch_array ($admin_role_mst_max_sort_order);
        ...
        if ($previous_parent_id != $role_parent_id) {
            $adminrole_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order - 1) WHERE  sort_order > ".$previous_sort_order."  AND parent_id =".$previous_parent_id."";
            $adminrole_mst_update_sort_order     = db_query($adminrole_mst_update_sort_order_sql);
        } else {

            $admin_role_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order - 1) WHERE  sort_order > ".$previous_sort_order." AND sort_order <= ".$sort_order." AND parent_id = " . $role_parent_id . " "; 
        ...
        $previous_parent_id  = $this->get_admin_role_master_parent_id ($id);
        if ($previous_parent_id != $role_parent_id) {
            $adminrole_mst_update_sort_order_sql1 = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order - 1) WHERE  sort_order > ".$previous_sort_order."  AND parent_id = " . $previous_parent_id . " ";
            $adminrole_mst_update_sort_order1     = db_query($adminrole_mst_update_sort_order_sql1);
            $adminrole_mst_update_sort_order_sql2 = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order + 1) WHERE  sort_order >= ".$sort_order." AND parent_id =".$role_parent_id."";
            $adminrole_mst_update_sort_order2     = db_query($adminrole_mst_update_sort_order_sql2);
        }else {
            if ($previous_sort_order < $sort_order ) {
                $adminrole_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order - 1) WHERE  sort_order > " . $previous_sort_order . " AND sort_order <= " . $sort_order . " AND parent_id =".$role_parent_id."";
                $adminrole_mst_update_sort_order     = db_query($adminrole_mst_update_sort_order_sql);
            } else if ($previous_sort_order > $sort_order) {
                $adminrole_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order + 1)WHERE  sort_order >=" . $sort_order . " AND sort_order < " . $previous_sort_order . " AND parent_id = " . $role_parent_id . " ";
                $adminrole_mst_update_sort_order     = db_query($adminrole_mst_update_sort_order_sql);
            }
        }
    }
    $admin_role_mst_update_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET parent_id = " . $role_parent_id . ", name = '" . $role_name . "', short_descr = '" . addslashes($short_desc) . "', sort_order = '" . $sort_order . "' , active = '" . $active . "', date_modified = NOW() WHERE id = '" . $id . "' ";
    $admin_role_mst_update     = db_query($admin_role_mst_update_sql);

    $admin_module_to_role_delete_sql = "DELETE FROM ".TABLE_MODULE_ROLE." WHERE admin_role_mst_id='".(int)$id."' ";
...
function get_admin_roles_ids_for_delete ($id)
{
    $all_id='';
    if ($id>0) {
        $privilege_role_child_result = db_query("SELECT id, parent_id, sort_order FROM " . TABLE_ROLE_MASTER ." WHERE parent_id = '" . $id . "'");
...
function get_admin_users_ids_for_delete ($role_id_string)
{
    if (not_null($role_id_string)) {
        $all_userid="";
        $admin_mst_sql = "SELECT id FROM " . TABLE_ADMIN_MASTER ." WHERE  admin_role_mst_id IN ('".$role_id_string."')";
 ...
 function privilegeRolesDelete($id, $parentid) {
    ...
    $this->update_remaining_privilege_sortorder($id, TABLE_ROLE_MASTER, $module_sql_cond);
    if (not_null ($id_string )) {
        $admin_id_string = $this->get_admin_users_ids_for_delete ($id_string);
        if (not_null ($admin_id_string)) {
            $admin_module_event_delete_sql = "DELETE FROM " .TABLE_MODULE_EVENT_ADMIN. " WHERE admin_mst_id  IN (".$admin_id_string.")";
            $admin_module_event_delete = db_query ($admin_module_event_delete_sql);
            $admin_module_delete_sql = "DELETE FROM " .TABLE_MODULE_ADMIN. " WHERE admin_mst_id  IN (".$admin_id_string.")";
            $admin_module_delete = db_query ($admin_module_delete_sql);
            $admin_master_delete_sql = "DELETE FROM " .TABLE_ADMIN_MASTER. " WHERE id  IN (".$admin_id_string.")";
            $admin_master_delete = db_query ($admin_master_delete_sql);
            $role_ids_arr = explode(',', $id_string);            
        } 
        $admin_module_to_role_delete_sql = "DELETE FROM " .TABLE_MODULE_ROLE. " WHERE admin_role_mst_id IN ('".$id_string."')";
        $admin_module_to_role_delete = db_query ($admin_module_to_role_delete_sql); 
        $admin_role_delete_sql = "DELETE FROM " .TABLE_ROLE_MASTER. " WHERE id IN ('".$id_string."')";
...
{
     $adminrole_mst_max_sort_order_sql    = "SELECT MAX(sort_order) as sort_order FROM " . TABLE_ROLE_MASTER . " WHERE parent_id=".$parentid."";
...
function adminRoleCheckDuplicate ($role_name, $role_parent_id) {
        $admin_role_check_duplicate_sql = "SELECT id FROM ".TABLE_ROLE_MASTER." WHERE name  = '" . trim($role_name) . "' AND parent_id =".$role_parent_id."";
...
function privilegeRolesUpdate($role_name, $role_parent_id, $id) {
        $admin_role_check_duplicate_sql = "SELECT id FROM ".TABLE_ROLE_MASTER." WHERE name  = '" . trim($role_name) . "' AND parent_id =".$role_parent_id." and id != ".$id."";
        $admin_role_check_duplicate = db_query($admin_role_check_duplicate_sql);
...
{
    $admin_module_mst_sql = "SELECT id, parent_id, name from ". TABLE_MODULE_MASTER ." WHERE parent_id=".(int)$parent_id." AND active='Y'";
    ...
    if ($tot_rec > 0) {
        if ($role_edit_id > 0) { //during editing time
            ...
            $admin_module_to_role_sql = "SELECT * from ".TABLE_MODULE_ROLE." WHERE  admin_role_mst_id='".(int)$role_edit_id."' AND admin_modules_mst_id IN (".$child_id_string.")";
...
function adminroleGenerateSortOrderValues($parent_id) {
        $adminrole_sort_order_sql = "SELECT MAX(sort_order) as sort_order FROM ".TABLE_ROLE_MASTER." WHERE parent_id =".$parent_id." ";
...
function adminRoleUpdateActive($value, $id, $parentid) {
            $adminrole_mst_update_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET `active` = '" . $value . "', `date_modified` = NOW() WHERE `id` = '" . $id . "' AND parent_id=".$parentid."";
...
function adminRoleUpdateSortOrder($id, $parentid, $value) {
          ...
          $previous_sort_order = $this->getAdminRoleMasterSortNumber($id);
            ...
            if ($previous_sort_order < $sort_order) {
                $adminrole_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order - 1) WHERE  sort_order > ".$previous_sort_order." AND sort_order <= ".$sort_order." AND parent_id = " . $parentid." ";
                $adminrole_mst_update_sort_order = db_query ($adminrole_mst_update_sort_order_sql);
            } else if ($previous_sort_order > $sort_order ) {
                $adminrole_mst_update_sort_order_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order = (sort_order + 1)WHERE  sort_order >=".$sort_order." AND sort_order < ".$previous_sort_order." AND parent_id = " . $parentid."";
                $adminrole_mst_update_sort_order = db_query ($adminrole_mst_update_sort_order_sql);
            }
            $adminrole_mst_update_sql = "UPDATE " . TABLE_ROLE_MASTER . " SET sort_order ='" . $sort_order . "',  date_modified = NOW() WHERE id= '" . $id . "' AND parent_id = " . $parentid . " ";
            $adminrole_mst_update = db_query ($adminrole_mst_update_sql);
}

function adminroleMstSql($parentid) {

        $adminrole_mst_sql = "SELECT * from " . TABLE_ROLE_MASTER . " WHERE parent_id = ".$parentid." order by sort_order";
...
{
    $admin_role_previous_parent_sql = "SELECT parent_id FROM ".TABLE_ROLE_MASTER." WHERE id='".$current_parentid."'";
...
{
    $adminrole_mst_parent   = db_query("select id from " . TABLE_ROLE_MASTER . " where parent_id = '" . (int)$id . "' LIMIT 0, 1");
...
{
            $admin_role_master_sql    = "SELECT * FROM " . TABLE_ROLE_MASTER . " WHERE id = " . $id ;
...
function adminModulesToAdminRoleMstSql ($id) 
    {
            $admin_modules_to_admin_role_mst_sql ="SELECT * from ".TABLE_MODULE_ROLE." WHERE admin_role_mst_id='".$id."' ";
