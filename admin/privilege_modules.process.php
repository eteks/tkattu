...
switch($_GET["action"]) {
    case "update_module_sortorder_combo":
        $tmp_max_sort_order = (($_GET["addflag"]=='1')?$privilegeModulesClass->getPrivilegeMaxSortOrder(TABLE_MODULE_MASTER, $module_sql_cond):((not_null($_GET["previous_parent_id"]) && $_GET["previous_parent_id"] != $module_parent_id )?$privilegeModulesClass->getPrivilegeMaxSortOrder(TABLE_MODULE_MASTER, $module_sql_cond) :$privilegeModulesClass->getPrivilegeMaxSortOrder(TABLE_MODULE_MASTER, $module_sql_cond)));
...
if ( db_num_rows ( $modules_master) ) {
    ?>
    <form name="frmPrivilegeModules" method="POST" action="<?php echo href_link(FILENAME_PRIVILEGE_MODULES,'parent_id='.$module_parent_id); ?>"><input type="hidden" name="action"><input type="hidden" name="id">
    ...
    <tr class="<?php echo $bgcolor;?>">
       ...
       <?php
            $modules_childs = $privilegeModulesClass->noOfChildOf($modules_master_values["id"], TABLE_MODULE_MASTER);
            if ($modules_childs>0) {
                ?><a title="view submodules" href="<?php echo href_link(FILENAME_PRIVILEGE_MODULES,'parent_id='.$modules_master_values["id"]); ?>"><?php
        ...
        <td style="text-align:center" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>"> <a title="edit module" href="<?php echo href_link(FILENAME_PRIVILEGE_MODULES, 'parent_id=' . $module_parent_id . '&id=' . $modules_master_values["id"] . '&action=privilege_module_edit'); ?>"><img src="images/edit.gif" width="15" height="15" alt="Edit" border="0"></a></td>
    ...
    print "<center><span class=\"normaltext\">NO MODULES FOUND</span></center>";
}
?>
