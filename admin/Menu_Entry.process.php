...
switch ($action) {
    case "menu_entry_update_active":
        $hms_info_obj->menuEntryUpdateActive(); 
    break;
    ...
    case "menu_entry_update":
        echo $hms_info_obj->checkDuplicate('update',$_GET['id']);  
        exit;
    break;
}
    $hms_info_db_num_rows = 0;
    $page_num_label = "&nbsp;";
    $hms_info_total_num_rows = $hms_info_obj->getMenuEntryTotalRecords();
    $true = 1;
    ...
    while ($true) {
        ...
        $hms_info_sql = $hms_info_obj->menuEntryFetchAllRecords($start_limit,$limit_rows);
...
?>
<form name="frmnewsletter_manageRecords" method="get" action="<? echo href_link(FILENAME_MENU_ENTRY);?>" style="margin:0px;">
      ...
      <tr>
        <td  colspan="11" ><table border="0" cellspacing="0" cellpadding="0">
			<tr>
			  ...
			  <td align="left" height="20" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_MENU_ENTRY;?></td>
			</tr>
		  </table></td>
      </tr>
   <tr>
    ...
    <tr>
         <th height="27" width="3%" style="text-align:center" ><input type="checkbox" name="All" id="All" class="noneborder" title="Check / Uncheck all" onclick="toggle_all_box(document.frmnewsletter_manageRecords, this.checked);set_button_stat(document.frmnewsletter_manageRecords, 'All', document.frmnewsletter_manageRecords.Delete_Selected);"></th>
                 <th width="14%" style="text-align:center" >Menu Category</th>
                 <th width="8%" style="text-align:center" >Item Code</th>
		  <th width="14%" style="text-align:center" >Menu Name</th>
                    <th width="14%" style="text-align:center" >Menu Price</th>
    ...
    <?php    
        ...
        $menu_type_sql = $hms_info_obj->MenuCategorySingRec($hms_info_values["menu_category_id"]);
        if ($hms_info_counter > ROWS_PER_PAGE) break;
              $bgcolor = (($hms_info_counter % 2 == 0) ? FIRSTROWCOLOR : SECONDROWCOLOR);
            $departmentdisp =   $hms_info_obj->departmentname($hms_info_values["depart_id"]);
    ?> 
    <tr>
        <td width="3%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><input type="checkbox" class="noneborder" onclick="change_box_stat(document.frmnewsletter_manageRecords, 'All');set_button_stat(document.frmnewsletter_manageRecords, 'All', document.frmnewsletter_manageRecords.Delete_Selected);" name="newsletter_manage_ids[]" value="<? print $hms_info_values["menu_id"]; ?>"></td>
	
       <td style="text-align:center" width="14%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["hms_menu_category_name"]) > 30) echo substr(stripslashes($hms_info_values["hms_menu_category_name"]),0,28).".."; else echo stripslashes($hms_info_values["hms_menu_category_name"]); ?></td>
        
	<td style="text-align:center" width="8%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["item_code"]) > 30) echo substr(stripslashes($hms_info_values["item_code"]),0,28).".."; else echo stripslashes($hms_info_values["item_code"]); ?></td>
		
        <td style="text-align:center" width="14%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["menu_name"]) > 30) echo substr(stripslashes($hms_info_values["menu_name"]),0,28).".."; else echo stripslashes($hms_info_values["menu_name"]); ?></td>
        
         <td style="text-align:center" width="14%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["menu_price"]) > 30) echo substr(stripslashes($hms_info_values["menu_price"]),0,28).".."; else echo stripslashes($hms_info_values["menu_price"]); ?></td>
		  
<?php /* ?> <td style="text-align:center" width="14%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["tax"]) > 30) echo substr(stripslashes($hms_info_values["tax"]),0,28).".."; else echo stripslashes($hms_info_values["tax"]); ?></td><?php */ ?>
		...
		<td width="6%" align="center" bgcolor="<? echo $bgcolor ?>" ><a href="javascript:populateHotelInfoManageLists ('<?php echo $hms_info_values["menu_id"] ?>', 'menu_entry_update_active', '<?php echo (($hms_info_values["active"] == "Y") ? "N" : "Y"); ?>', '<?php print $page ;?>', '');"><img src="images/<?php echo $hms_info_values["active"]; ?>.gif" border="0" <? if ($hms_info_values["active"]=='Y'){print "title='Click to disable'";} else { print "title='Click to enable'";}?>></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<?php echo FILENAME_MENU_ENTRY_VIEW . '?id=' . $hms_info_values["menu_id"]; ?>" onClick="return hs.htmlExpand(this, { objectType: 'ajax'} )"><img src="images/view.gif" width="16" height="16" border="0" /></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<?php echo href_link(FILENAME_MENU_ENTRY, 'id=' . $hms_info_values["menu_id"] . '&action=menu_entry_edit&page=' . $page) ?>"><img src="images/edit.gif" alt="Edit" border="0" title="Click to edit"></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="javascript:deleteHmsInfoManageRecord(document.frmnewsletter_manageRecords, 'menu_entry_manage_delete', '<?php echo $hms_info_values["menu_id"] ?>')"><img src="images/delete.gif" alt="Delete" border="0" title="Click to delete"></a></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
  <tr>
   <td colspan="10" height="30" class="gray_headline"><table border="0" cellspacing="0" cellpadding="0">
     <tr>
       <td style="padding-left:6px"><img src="images/arrow_heading.gif" /></td>
       <td align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_MENU_ENTRY;?></td>
     </tr>
   </table></td>
  </tr>
  <tr>
