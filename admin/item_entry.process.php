...
switch ($action) {
    case "item_update_active":
        $hms_info_obj->itemUpdateActive();
    break;
    case "item_update_live_active":
        $hms_info_obj->itemUpdateLive();
    break;
}
    ...
    while ($true) {
        ...
        $hms_info_sql = $hms_info_obj->getitemInfoFetchAllRecords($start_limit,$limit_rows);		
...
?>
<form name="frmnewsletter_manageRecords" method="get" action="<? echo href_link(FILENAME_ENTRY_TYPE);?>" style="margin:0px;">
      ...
      <tr>
        <td  colspan="11" ><table border="0" cellspacing="0" cellpadding="0">
			<tr>
			  ...
			  <td align="left" height="20" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_ITEM_ENTRY;?></td>
			</tr>
		  </table></td>
      </tr>
   <tr>
      ...
      <tr>
         ...
         <table width="100%" border="0" cellspacing="0" cellpadding="5" class="ntab">
    <tr>
         ...
         <!--<th width="13%" style="text-align:center" >Vendor Name</th>-->
         <th width="12%" style="text-align:center" >Item  Name</th>
		  <th width="13%" style="text-align:center" >Item Type </th>
		   <th width="12%" style="text-align:center" >Unit </th>
            <!--<th width="8%" style="text-align:center" >Price/Qty</th>
		    <th width="15%" style="text-align:center" >Purchased Qty</th>
    ...
    <?php    
        ...
        if ($hms_info_counter > ROWS_PER_PAGE) break;
              ...
              $itemtypename = $hms_info_obj->itemtypename($hms_info_values["item_entry_type"]) ;
              $unitname = $hms_info_obj->unitname($hms_info_values["item_unit"]) ;
    ?> 
    <tr>
        <td width="3%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><input type="checkbox" class="noneborder" onclick="change_box_stat(document.frmnewsletter_manageRecords, 'All');set_button_stat(document.frmnewsletter_manageRecords, 'All', document.frmnewsletter_manageRecords.Delete_Selected);" name="newsletter_manage_ids[]" value="<? print $hms_info_values["item_entry_id"]; ?>"></td>
	
        <!--<td width="8%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["vendor_name"]) > 30) echo substr(stripslashes($hms_info_values["vendor_name"]),0,28).".."; else echo stripslashes($hms_info_values["vendor_name"]); ?></td>-->
        
       <td width="8%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
       
        <? if(strlen($hms_info_values["item_entry_name"]) > 30) echo substr(stripslashes($hms_info_values["item_entry_name"]),0,28).".."; else echo stripslashes($hms_info_values["item_entry_name"]); ?></td>
        
		  <td width="10%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
		    <? if(strlen($itemtypename) > 30) echo substr($itemtypename,0,28).".."; else echo $itemtypename; ?></td>


		  <td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($unitname) > 30) echo substr($unitname,0,28).".."; else echo $unitname; ?></td>

		 

		  <!--<td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["standard_rate"]) > 30) echo substr($hms_info_values["standard_rate"],0,28).".."; else echo $hms_info_values["standard_rate"]; ?></td>-->


		 <!-- <td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["standard_qty"]) > 30) echo substr($hms_info_values["standard_qty"],0,28).".."; else echo $hms_info_values["standard_qty"]; ?></td>
        -->
        <!--<td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["item_maxqty"]) > 30) echo substr($hms_info_values["item_maxqty"],0,28).".."; else echo $hms_info_values["item_maxqty"]; ?></td>-->


      
		<!--<td width="5%" align="center" bgcolor="<? echo $bgcolor ?>" > <a href="javascript:populateHotelInfoManageLists ('<? echo $hms_info_values["item_entry_id"] ?>', 'item_update_active', '<? echo (($hms_info_values["active"] == "Y") ? "N" : "Y"); ?>', '<? print $page ;?>', '');"><img src="images/<? echo $hms_info_values["active"]; ?>.gif" border="0" <? if ($hms_info_values["active"]=='Y'){print "title='Click to disable'";} else { print "title='Click to enable'";}?>></a></td>-->

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo FILENAME_ENTRY_TYPE_VIEW. '?id=' . $hms_info_values["item_entry_id"]; ?>" onClick="return hs.htmlExpand(this, { objectType: 'ajax'} )"><img src="images/view.gif" width="16" height="16" border="0" /></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo href_link(FILENAME_ENTRY_TYPE, 'id=' . $hms_info_values["item_entry_id"] . '&action=item_edit&page=' . $page) ?>"><img src="images/edit.gif" alt="Edit" border="0" title="Click to edit"></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="javascript:deleteHmsInfoManageRecord(document.frmnewsletter_manageRecords, 'item_manage_delete', '<? echo $hms_info_values["item_entry_id"] ?>')"><img src="images/delete.gif" alt="Delete" border="0" title="Click to delete"></a></td>
  </tr>
    <?php
          $hms_info_counter++;
        }
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
  <tr>
   <td colspan="10" class="gray_headline"><table border="0" cellspacing="0" cellpadding="0">
     <tr>
       <td style="padding-left:6px"><img src="images/arrow_heading.gif" /></td>
       <td align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_ITEM_ENTRY;?></td>
     </tr>
   </table></td>
  </tr>
  <tr>
