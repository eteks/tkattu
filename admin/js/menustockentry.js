...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "menustockentry.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    ...
    if (frm.mse_cate.selectedIndex>0) {
   msg('error_mse_cate', "tahoma10rednormal", "");
    } else {
        msg ('error_mse_cate', "tahoma10rednormal", "ERROR: required");
        errs +=1;
        setfocus(frm.mse_cate);
    }
     if (frm.mse_menu.selectedIndex>0) {
        msg('error_mse_menu', "tahoma10rednormal", "");
    } else {
        msg ('error_mse_menu', "tahoma10rednormal", "ERROR: required");
        errs +=1;
        setfocus(frm.mse_menu);
    } 	
    if (errs==0 && !commonCheck(obj.mse_qty,'error_mse_qty',true)) errs +=1;
