...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "item_entry.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    frm = obj;
    var errs=0;
    if (!commonCheck(obj.item_entry_name, 'error_item_entry_name',true)) errs +=1;
	 if (!commonCheck(obj.item_entry_type, 'error_item_entry_type',true)) errs +=1;
	  if (!commonCheck(obj.item_unit,'error_item_unit',true)) errs +=1;
	   if (!commonCheck(obj.opening_stock, 'error_opening_stock',true)) errs +=1;
	    if (!commonCheck(obj.item_maxqty, 'error_item_maxqty',true)) errs +=1;
	     if (!commonCheck(obj.item_minqty, 'error_item_minqty',true)) errs +=1;
		  if (!commonCheck(obj.standard_qty, 'error_standard_qty',true)) errs +=1;
		    if (!commonCheck(obj.standard_rate, 'error_standard_rate',true)) errs +=1;
