...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "tax_scheme.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    frm = obj;
    var errs=0;
    if (!commonCheck(obj.scheme_name,'error_scheme_name',true)) errs +=1;
    //if (!validateDate(obj, 'pub', 'error_pub_date') && errs<=0 ) errs += 1;
    //if (!commonCheck(obj.payment_name,'error_payment_name',true)) errs +=1;
