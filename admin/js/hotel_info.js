...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "hotel_info.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    frm = obj;
    var errs=0;
    if (errs == 0 && !commonCheck(obj.hms_info_name,'error_hms_info_name',true)) errs +=1;
    if (errs == 0 && !commonCheck(obj.hms_info_address, 'error_hms_info_address', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.hms_info_city, 'error_hms_info_city', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.hms_info_state, 'error_hms_info_state', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.hms_info_zip, 'error_hms_info_zip', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.country, 'error_country', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.hms_info_phone, 'error_hms_info_phone', true)) errs += 1;
    if (errs == 0 && !commonCheck(obj.hms_info_fax, 'error_hms_info_fax', true)) errs += 1;
    if (errs == 0 && !validateEmail(obj.hms_info_email, 'error_hms_info_email', true)) errs += 1;
    if (errs == 0 && !validateWebsite(obj.hms_info_url, 'error_hms_info_url', true)) errs += 1;
