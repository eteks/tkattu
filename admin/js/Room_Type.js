...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "Room_Type.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    ...
    frm = obj;
    var errs=0;
    if (!commonCheck(obj.room_type,'error_room_type',true)) errs +=1;
    if (!commonCheck(obj.bed_size,'error_bed_size',true)) errs +=1;
    if (!commonCheck(obj.charge,'error_charge',true)) errs +=1;
    ...
    if (errs == 0) {
        var reqFaq1 = new XMLHttpRequest();
            ...
            if (reqFaq1.readyState == 4) {
                  if (reqFaq1.status == 200) { alert(reqFaq1.responseText);
                      if (reqFaq1.responseText == "1") {
                        ...
                        if (frm.action.value == "room_type_insert") {
         ...
         var pars = "id=" + obj.id.value + "&page=" + obj.page.value + "&active=" + obj.active.value + "&action=" + obj.action.value + "&room_type=" + obj.room_type.value;
         reqFaq1.open("GET", "Room_Type.process.php?" + pars, true);
         reqFaq1.send("");
