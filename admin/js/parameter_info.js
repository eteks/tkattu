...
function submitfrmStudentUser(obj) {
    frm = obj;
	if((document.frmStudentUser.hms_hotel_name.value)=='')
	{
		alert("Enter the all missing Field:");
	}
    var errs=0;
	...
	if (errs == 0 && !commonCheck(obj.hms_url, 'error_hms_url', true)) errs += 1;
	if (errs == 0 && !commonCheck(obj.hms_email, 'error_hms_email', true)) errs += 1;
	if (errs == 0 && !commonCheck(obj.hms_footertxt, 'error_hms_footertxt', true)) errs += 1;
