...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "Facility_Entry.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    ...
    frm = obj;
    var errs=0;
    if (errs==0 && !commonCheck(obj.facility_name,'error_facility_name',true)) errs +=1;
    ...
    if (errs == 0) {
        var reqFaq1 = new XMLHttpRequest();
            ...
            if (reqFaq1.readyState == 4) {
                  if (reqFaq1.status == 200) { //alert(reqFaq1.responseText);
                      if (reqFaq1.responseText == "1") {
                        ...
                        if (frm.action.value == "facility_entry_insert") {
                            alert("Success: Facility Entry added successfully");
                        } else {
                            alert("Success: Facility Entry updated successfully");
                        }
                        frm.submit();
                      } else {
                          alert("Error: Duplicate record found");
                          setfocus(frm.facility_name);
                      }
                 } else {
                      alert("Error: While trying to process whatever you requested");
                 }
         ...
         var pars = "id=" + obj.id.value + "&page=" + obj.page.value + "&active=" + obj.active.value + "&action=" + obj.action.value + "&facility_name=" + obj.facility_name.value;
         reqFaq1.open("GET", "Facility_Entry.process.php?" + pars, true);
         reqFaq1.send("");
