...
function submitfrmPrivilegeUsers(obj) {
    ...
    if (errs == 0) {
        ...
        requsersubmit.open("GET", "hms_privilege_users.process.php?id="+id+"&uname="+escape(trim(obj.uname.value))+"&action=privilege_user_duplication", true);
        requsersubmit.send("");
    }
    return false;
}
...
function populate_privilege_users_records(page, role_id, id, action, value) {   
    ...
    reqPrivilege1.open("GET", "hms_privilege_users.process.php?page="+page+"&parent_id="+role_id+"&id="+id+"&action="+action+"&value="+value, true);
    reqPrivilege1.send("");
}
...
function update_roles_modules_records(placeid, role_id, addflag, id) {
    if (document.frmPrivilegeUsers.parent_id.selectedIndex>0) {
        ...
        reqPrivilege1.open("GET", "hms_privilege_users.process.php?parent_id="+role_id+"&action=update_role_modules_tree&addflag="+addflag+"&id="+id, true);
        reqPrivilege1.send("");
    } else {
...
function update_user_modules_records(obj, placeid, role_id, module_id) {
    ...
    if (obj.checked) {
        ...
        reqPrivilege1.open("GET", "hms_privilege_users.process.php?parent_id="+role_id+"&action=update_user_modules_tree&module_id="+module_id+"&addflag="+addflag+"&id="+id, true);
        reqPrivilege1.send("");
    } else {
        document.getElementById(placeid).innerHTML='';
...
function populate_privilege_modules_records(parent_id, id, action, value) {
    var glbDivId1 = document.getElementById("hms_privilege_modules_records");
    var reqPrivilege1 = new XMLHttpRequest();
    reqPrivilege1.onreadystatechange = function () {
        if (reqPrivilege1.readyState == 4) {
            if (reqPrivilege1.status == 200) {
    ...
    reqPrivilege1.open("GET", "hms_privilege_modules.process.php?parent_id="+parent_id+"&id="+id+"&action="+action+"&value="+value, true);
    reqPrivilege1.send("");
}
function update_modules_sortorder_records(parent_id, id, addflag, previous_parent_id) {
    ...
    reqPrivilege1.open("GET", "hms_privilege_modules.process.php?parent_id="+parent_id+"&action=update_module_sortorder_combo&id="+id+"&addflag="+addflag+"&previous_parent_id="+previous_parent_id, true);
    reqPrivilege1.send("");
}
...
function populate_privilege_events_records(event_module_parent_id) {  
    ...
    if (event_module_parent_id==0) {
        EventsDivId.innerHTML = "<center><span style='color:#000000;'><b>Please Select Module Name</b></span></center>";
    } else {        
       ...
       reqPrivEvent.open("GET", "hms_privilege_events.process.php?event_module_parent_id="+event_module_parent_id, true);
       reqPrivEvent.send("");
    }
}
...
function submitfrmPrivilegeAdminRoles(obj) {   
    ...
    if (errs == 0) {           
        ...
        reqrolesubmit.open("GET", "hms_privilege_roles.process.php?id="+obj.id.value+"&role_parent_id="+obj.role_parent_id.value+"&role_name="+obj.role_name.value+"&action="+obj.action.value, true);
        reqrolesubmit.send("");
    }
...
function get_admin_module_child_checkbox (child_id) {   
    ...
    if (!document.getElementById('parent'+child_id).checked) {
        ...
        reqchild.open("GET", "hms_privilege_roles.process.php?child_id="+child_id+"&action=admin_role_child_names&role_edit_id="+role_edit_id, true);
        reqchild.send("");
   }
}
...
function populateAdminRoleSortOrderValues(parentid, sort_selectedvalue) {       
    ...
    req1.open("GET", "hms_privilege_roles.process.php?parent_id="+parentid+"&action=adminrole_generate_sort_order_values", true);
    req1.send("");
}
...
function populate_privilege_roles_records(parentid) { 
    ...
    req3.open("GET", "hms_privilege_roles.process.php?&parentid="+parentid, true);
    req3.send("");
...
function populate_admin_role_records(id, action, value, parentid) {  
    ...
    req4.open("GET", "hms_privilege_roles.process.php?&id="+id+"&action="+action+"&value="+value+"&parentid="+parentid, true);
    req4.send("");
}