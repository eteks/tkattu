...
function populateHotelInfoManageLists(id, action, value, page) {
    ...
    reqStudUser.open("GET", "vendor_creation.process.php?" + pars, true);
    reqStudUser.send("");
}
...
function submitfrmStudentUser(obj) {
    frm = obj;
    var errs=0;
    if (errs == 0 && !commonCheck(obj.vendor_name,'error_vendor_name',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_address,'error_vendor_address',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_city,'error_vendor_city',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_state,'error_vendor_state',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_country,'error_vendor_country',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_zip,'error_vendor_zip',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_phone,'error_vendor_phone',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_mobile,'error_vendor_mobile',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.vendor_contact,'error_vendor_contact',true)) errs +=1;
	if (errs == 0 && !commonCheck(obj.item_id,'error_item_id',true)) errs +=1;
