...
switch ($action) {
    case "room_update_active":
        $hms_info_obj->roomUpdateActive();
    break;
    case "room_insert":
        echo '1';
        exit;
    ...
    $page_num_label = "&nbsp;";
    $hms_info_total_num_rows = $hms_info_obj->getRoomTotalRecords();
    $true = 1;
    $page = $_POST["page"] ? $_POST["page"] : $_GET["page"];
    while ($true) {
        $start_limit = ((not_null($page) && $page>1) ? (($page-1) * ROWS_PER_PAGE) : '0');
        $limit_rows = ROWS_PER_PAGE;
        $hms_info_sql = $hms_info_obj->roomFetchAllRecords($start_limit,$limit_rows);
...
?>
<form name="frmnewsletter_manageRecords" method="get" action="<? echo href_link(FILENAME_ROOM);?>" style="margin:0px;">
      ...
      <tr>
        <td  colspan="11" ><table border="0" cellspacing="0" cellpadding="0">
			<tr>
			  ...
			  <td align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_ROOM;?></td>
			</tr>
		  </table></td>
      </tr>
   <tr>
    ...
    <tr>
         ...
         <th width="9%" style="text-align:center" >Room No</th>
         <th width="21%" style="text-align:center" >Room Type</th>
    ...
    <tr>
		...
		<td width="5%" align="center" bgcolor="<? echo $bgcolor ?>" ><a href="javascript:populateHotelInfoManageLists ('<? echo $hms_info_values["room_id"] ?>', 'room_update_active', '<? echo (($hms_info_values["active"] == "Y") ? "N" : "Y"); ?>', '<? print $page ;?>', '');"><img src="images/<? echo $hms_info_values["active"]; ?>.gif" border="0" <? if ($hms_info_values["active"]=='Y'){print "title='Click to disable'";} else { print "title='Click to enable'";}?>></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo FILENAME_ROOM_VIEW . '?id=' . $hms_info_values["room_id"]; ?>" onClick="return hs.htmlExpand(this, { objectType: 'ajax'} )"><img src="images/view.gif" width="16" height="16" border="0" /></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo href_link(FILENAME_ROOM, 'id=' . $hms_info_values["room_id"] . '&action=room_edit&page=' . $page) ?>"><img src="images/edit.gif" alt="Edit" border="0" title="Click to edit"></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="javascript:deleteHmsInfoManageRecord(document.frmnewsletter_manageRecords, 'room_delete', '<? echo $hms_info_values["room_id"] ?>')"><img src="images/delete.gif" alt="Delete" border="0" title="Click to delete"></a></td>
