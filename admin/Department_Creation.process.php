...
switch ($action) {
    case "department_entry_update_active":
        $hms_info_obj->departmentUpdateActive();
    break;
}
...
?>
<form name="frmnewsletter_manageRecords" method="get" action="<? echo href_link(FILENAME_DEPARTMENT_CREATION);?>" style="margin:0px;">
      ...
      <tr>
        <td  colspan="11" ><table border="0" cellspacing="0" cellpadding="0">
			<tr>
			  ...
			  <td align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_DEPARTMENT_CREATION;?></td>
			</tr>
		  </table></td>
      </tr>
   <tr>
    ...
    <tr>
         ...
         <th width="30%" style="text-align:center" >Departmenmt Name</th>
    ...
    ?> 
    <tr>
        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><input type="checkbox" class="noneborder" onclick="change_box_stat(document.frmnewsletter_manageRecords, 'All');set_button_stat(document.frmnewsletter_manageRecords, 'All', document.frmnewsletter_manageRecords.Delete_Selected);" name="newsletter_manage_ids[]" value="<? print $hms_info_values["department_creation_id"]; ?>"></td>
	
        <td style="text-align:center" width="30%" class="tahoma11blacknormal" bgcolor="<? echo $bgcolor ?>" >
        <? if(strlen($hms_info_values["department_creation_name"]) > 30) echo substr(stripslashes($hms_info_values["department_creation_name"]),0,28).".."; else echo stripslashes($hms_info_values["department_creation_name"]); ?></td>
        <td width = "20%" style="text-align:center" class="tahoma11blacknormal" bgcolor="<?php echo $bgcolor; ?>"><?php echo date_long( $hms_info_values["created_date"] ); ?></td>

        <td width = "20%" style="text-align:center" class="tahoma11blacknormal" bgcolor="<?php echo $bgcolor; ?>"><?php echo date_long( $hms_info_values["modified_date"] ); ?></td>
		
		<td width="5%" align="center" bgcolor="<? echo $bgcolor ?>" ><a href="javascript:populateHotelInfoManageLists ('<? echo $hms_info_values["department_creation_id"] ?>', 'department_entry_update_active', '<? echo (($hms_info_values["active"] == "Y") ? "N" : "Y"); ?>', '<? print $page ;?>', '');"><img src="images/<? echo $hms_info_values["active"]; ?>.gif" border="0" <? if ($hms_info_values["active"]=='Y'){print "title='Click to disable'";} else { print "title='Click to enable'";}?>></a></td>
		
		
        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo FILENAME_DEPARTMENT_CREATION_VIEW . '?id=' . $hms_info_values["department_creation_id"]; ?>" onClick="return hs.htmlExpand(this, { objectType: 'ajax'} )"><img src="images/view.gif" width="16" height="16" border="0" /></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo href_link(FILENAME_DEPARTMENT_CREATION, 'id=' . $hms_info_values["department_creation_id"] . '&action=department_edit&page=' . $page) ?>"><img src="images/edit.gif" alt="Edit" border="0" title="Click to edit"></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="javascript:deleteHmsInfoManageRecord(document.frmnewsletter_manageRecords, 'department_manage_delete', '<? echo $hms_info_values["department_creation_id"] ?>')"><img src="images/delete.gif" alt="Delete" border="0" title="Click to delete"></a></td>
