...
switch ($action) {
    case "vendor_update_active":
        $hms_info_obj->vendorUpdateActive();
    break;
    case "vendor_update_live_active":
        $hms_info_obj->vendorUpdateLive();
    break;
}
    ...
    while ($true) {
        ...
        $hms_info_sql = $hms_info_obj->getvendorInfoFetchAllRecords($start_limit,$limit_rows);
...
?>
<form name="frmnewsletter_manageRecords" method="get" action="<? echo href_link(FILENAME_VENDOR_CREATION);?>" style="margin:0px;">
...
<tr><td></td></tr>
        ...
        <td colspan="11" >
            <table border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td height="20" align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_VENDOR_CREATION;?></td>
                </tr>
            </table>
        </td>
    </tr>
    ...
    <tr>
        ...
        <th width="17%" style="text-align:center" >Vendor  Name </th>
    ...
    ?>
    <tr>
        <td width="3%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><input type="checkbox" class="noneborder" onclick="change_box_stat(document.frmnewsletter_manageRecords, 'All');set_button_stat(document.frmnewsletter_manageRecords, 'All', document.frmnewsletter_manageRecords.Delete_Selected);" name="newsletter_manage_ids[]" value="<? print $hms_info_values["vendor_id"]; ?>"></td>

        <td width="17%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["vendor_name"]) > 30) echo ucwords(substr(stripslashes($hms_info_values["vendor_name"]),0,28)).".."; else echo ucwords(stripslashes($hms_info_values["vendor_name"])); ?></td>

        <td width="10%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["vendor_phone"]) > 30) echo substr(stripslashes($hms_info_values["vendor_phone"]),0,28).".."; else echo stripslashes($hms_info_values["vendor_phone"]); ?></td>

        <td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["vendor_city"]) > 30) echo ucwords(substr(stripslashes($hms_info_values["vendor_city"]),0,28)).".."; else echo stripslashes (ucwords($hms_info_values["vendor_city"])); ?></td>

        <td width="7%" bgcolor="<? echo $bgcolor ?>" class="tahoma11blacknormal" style="text-align:center" >
        <? if(strlen($hms_info_values["vendor_state"]) > 30) echo ucwords(substr(stripslashes($hms_info_values["vendor_state"]),0,28)).".."; else echo ucwords(stripslashes($hms_info_values["vendor_state"])); ?></td>

        <td width="5%" align="center" bgcolor="<? echo $bgcolor ?>" ><a href="javascript:populateHotelInfoManageLists ('<? echo $hms_info_values["vendor_id"] ?>', 'vendor_update_active', '<? echo (($hms_info_values["active"] == "Y") ? "N" : "Y"); ?>', '<? print $page ;?>', '');"><img src="images/<? echo $hms_info_values["active"]; ?>.gif" border="0" <? if ($hms_info_values["active"]=='Y'){print "title='Click to disable'";} else { print "title='Click to enable'";}?>></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo FILENAME_VENDOR_CREATION_VIEW. '?id=' . $hms_info_values["vendor_id"]; ?>" onClick="return hs.htmlExpand(this, { objectType: 'ajax'} )"><img src="images/view.gif" width="16" height="16" border="0" /></a></td>

        <td width="4%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="<? echo href_link(FILENAME_VENDOR_CREATION, 'id=' . $hms_info_values["vendor_id"] . '&action=vendor_edit&page=' . $page) ?>"><img src="images/edit.gif" alt="Edit" border="0" title="Click to edit"></a></td>

        <td width="5%" style="text-align:center" bgcolor="<? echo $bgcolor ?>"><a href="javascript:deleteHmsInfoManageRecord(document.frmnewsletter_manageRecords, 'vendor_manage_delete', '<? echo $hms_info_values["vendor_id"] ?>')"><img src="images/delete.gif" alt="Delete" border="0" title="Click to delete"></a></td>
    </tr>
    <?php
          $hms_info_counter++;
        }
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
  <tr>
   <td colspan="10" class="gray_headline"><table border="0" cellspacing="0" cellpadding="0">
     <tr>
       <td style="padding-left:6px"><img src="images/arrow_heading.gif" /></td>
       <td align="left" class="heading_light_blue">&nbsp;<?php echo TITLE_MANAGE_VENDOR_CREATION;?></td>
     </tr>
   </table></td>
  </tr>
  <tr>
