...
<strong><u>RESTAURANT TABLE SALES REPORT </u></strong></td>
  </tr>
<?php
...
<?php
  $tabletree = db_query("SELECT `table_entry_id`,`table_type_id`,`table_no`,`numbers_of_chairs`,`active`,`date_added`,`date_modified` FROM " . TABLE_HMS_TABLE_ENTRY . " WHERE `active` = 'Y' AND table_entry_id='".$_GET['tableid']."'");
while($table=db_fetch_array($tabletree)){
$table_no=$table['table_no'];
}  
  ?>
  <td colspan="2" align="left" class="NormalTxt"><strong>TABEL NO</strong>:&nbsp;<?php echo $table_no; ?></td>
    </tr>
    </table> 
  ...
  <tr align="center" bgcolor="#999999" class="site_font_black">
   ...
   <?php
   $Purchase_List_Details1 = "SELECT * FROM " . TABLE_HMS_RESTAURANT_ACCOUNT_DETAILS . " WHERE  tabel_id='".$_GET['tableid']."' AND orde_close_date BETWEEN '". $_GET["fromdd"] ."' AND '". $_GET["todd"] ."' AND  status = 'close' order by bill_id ASC";
