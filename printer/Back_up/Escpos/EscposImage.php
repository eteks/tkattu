    ...
    public static function load(
        ...
        foreach ($preferred as $implemetnation) {
            if ($implemetnation === 'imagick') {
                ...
                return new \printer\Escpos\ImagickEscposImage($filename, $allow_optimisations);
            } elseif ($implemetnation === 'gd') {
                if (!self::isGdLoaded()) {
                    // Skip option if GD not loaded
                    continue;
                }
                return new \printer\Escpos\GdEscposImage($filename, $allow_optimisations);
                ...
                return new \printer\Escpos\NativeEscposImage($filename, $allow_optimisations);
            } else {
